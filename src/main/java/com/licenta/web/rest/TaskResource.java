...
/**
 * REST controller for managing {@link com.licenta.domain.Task}.
 */
@RestController
@RequestMapping("/api")
public class TaskResource {
    ...
    @PostMapping("/tasks/project/{projectId}/sprint/{sprintId}/person/{personId}")
    public ResponseEntity<TaskDTO> createProjectPhase(
        @PathVariable final Long personId,
        @PathVariable final long sprintId,
        @PathVariable final long projectId,
        @Valid @RequestBody TaskDTO taskDTO
    ) throws URISyntaxException {
        log.debug("REST request to save Task : {}", taskDTO);

        TaskDTO result = taskService.saveTaskAndSprintAndPerson(taskDTO, projectId, sprintId, personId);
        return ResponseEntity
            .created(new URI("/api/tasks/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, result.getId().toString()))
            .body(result);
