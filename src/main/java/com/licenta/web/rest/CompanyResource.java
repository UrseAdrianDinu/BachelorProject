...
import com.licenta.service.CompanyService;
...
/**
 * REST controller for managing {@link com.licenta.domain.Company}.
 */
@RestController
@RequestMapping("/api")
public class CompanyResource {
    ...
    @PostMapping("/companies/add-company-role/{id}")
    public ResponseEntity<CompanyDTO> createCompanyAndAddRole(
        @PathVariable(value = "id", required = false) final Long id,
        @Valid @RequestBody CompanyRoleDTO companyRoleDTO
    ) throws URISyntaxException {
        log.debug("REST request to create Company and Role : {}", companyRoleDTO.getCompanyDTO().getName());
        if (companyRoleDTO.getCompanyDTO().getId() != null) {
            throw new BadRequestAlertException("A new company cannot already have an ID", ENTITY_NAME, "id exists");
        }
        CompanyDTO result = companyService.createCompanyAndAddRole(companyRoleDTO.getCompanyDTO(), companyRoleDTO.getRoleDTO(), id);
        return ResponseEntity
            .created(new URI("/api/companies/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, result.getId().toString()))
            .body(result);
    ...
    public List<DepartmentDTO> getCompanyDepartments(@PathVariable Long id) {
        log.debug("REST request to get Company Departments: {}", id);
        return companyService.findCompanyDepartments(id);
    }

    @GetMapping("/companies/department-names/{id}")
    public List<String> getCompanyDepartmentNames(@PathVariable Long id) {
        log.debug("REST request to get Company Departments: {}", id);
        return companyService.findCompanyDepartmentNames(id);
    }

    @GetMapping("/companies/projects/{id}")
    public List<ProjectDTO> getCompanyProjects(@PathVariable Long id) {
        log.debug("REST request to get Company Projects: {}", id);
        return companyService.findCompanyProjects(id);
    }

    @GetMapping("/companies/people/{id}")
    public List<PersonDTO> getCompanyPeople(@PathVariable Long id) {
        log.debug("REST request to get Company Projects: {}", id);
        return companyService.findCompanyPersons(id);
    }

    @GetMapping("/companies/people-users/{id}")
    public List<PersonUserDTO> getCompanyPersonUsers(@PathVariable Long id) {
        log.debug("REST request to get Company Projects: {}", id);
