...
/**
 * REST controller for managing {@link com.licenta.domain.Project}.
 */
@RestController
@RequestMapping("/api")
public class ProjectResource {
    ...
    @PostMapping("/projects/company/{id}")
    public ResponseEntity<ProjectDTO> createProjectAndAddCompany(
        @PathVariable final Long id,
        @Valid @RequestBody ProjectCreateDTO projectCreateDTO
    ) throws URISyntaxException {
        log.debug("REST request to save Project : {}", projectCreateDTO);

        ProjectDTO result = projectService.saveProjectAndAddCompany(projectCreateDTO, id);
        return ResponseEntity
            .created(new URI("/api/projects/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, result.getId().toString()))
            .body(result);
    ...
    public List<PhaseDTO> getProjectPhases(@PathVariable Long id) {
        log.debug("REST request to get Project Phases : {}", id);
        return projectService.findProjectPhases(id);
    }

    @GetMapping("/projects/{id}/teams")
    public List<TeamDTO> getProjectTeams(@PathVariable Long id) {
        log.debug("REST request to get Project Teams: {}", id);
        return projectService.findProjectTeams(id);
    }

    @PostMapping("/projects/{id}/phases")
    public List<PhaseDTO> createProjectPhase(@PathVariable Long id) {
        log.debug("REST request to get Project Phases : {}", id);
        return projectService.findProjectPhases(id);
    }

    @GetMapping("/projects/people-users/{id}")
    public List<PersonUserDTO> getProjectPeople(@PathVariable Long id) {
        log.debug("REST request to get Project People: {}", id);
        return projectService.findProjectPeople(id);
    }
