...
public class PersonRepositoryWithBagRelationshipsImpl implements PersonRepositoryWithBagRelationships {
    ...
    public Optional<Person> fetchBagRelationships(Optional<Person> person) {
        return person.map(this::fetchTeams);
    }
    ...
    public List<Person> fetchBagRelationships(List<Person> people) {
        return Optional.of(people).map(this::fetchTeams).orElse(Collections.emptyList());
    }

    Person fetchTeams(Person result) {
        return entityManager
            .createQuery("select person from Person person left join fetch person.teams where person is :person", Person.class)
            .setParameter("person", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Person> fetchTeams(List<Person> people) {
        ...
        List<Person> result = entityManager
            .createQuery("select distinct person from Person person left join fetch person.teams where person in :people", Person.class)
