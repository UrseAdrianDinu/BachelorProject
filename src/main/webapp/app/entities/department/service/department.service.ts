...
import { CreateDepartment } from '../create-department/create-department.model';
...
export class DepartmentService {
  ...
  createDeparmentCompany(department: CreateDepartment, companyId: number): Observable<EntityResponseType> {
    return this.http.post<IDepartment>(`${this.resourceUrl}/company/${companyId}`, department, { observe: 'response' });
  }
  ...
  queryByCompany(companyId: number, req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<IDepartment[]>(`${this.resourceUrlCompany}/departments/${companyId}`, { params: options, observe: 'response' });
  }
